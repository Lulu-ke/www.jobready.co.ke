...
import { Briefcase, Loader2, AlertCircle, ChevronRight, Wifi } from 'lucide-react';
...
interface TypeJobsClientProps {
  jobType: string;
  typeSlug: string;
}

function getTypeIcon(type: string) {
  if (type === 'Remote') return <Wifi className="w-8 h-8 text-teal-600" />;
  return <Briefcase className="w-8 h-8 text-teal-600" />;
}

function TypeJobsSkeleton() {
  return (
...
function TypeJobsInner({ jobType, typeSlug }: TypeJobsClientProps) {
  ...
  // Fetch jobs pre-filtered by type
  ...
  const heroTitle = jobType === 'Remote' 
    ? 'Remote & Work From Home Jobs in Kenya' 
    : `${jobType} Jobs in Kenya`;

  const emptyMessage = jobType === 'Remote'
    ? 'No remote jobs currently available'
    : `No ${jobType.toLowerCase()} jobs currently available`;
      ...
      <section className="bg-gradient-to-br from-white to-gray-50 py-10 lg:py-14 border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <nav className="flex items-center gap-1.5 text-sm text-gray-400 mb-6">
            ...
            <ChevronRight className="w-3.5 h-3.5" />
            <span className="text-gray-700 font-medium">{jobType}</span>
          </nav>
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-gray-500 max-w-xl mx-auto">
              Browse {total > 0 ? total.toLocaleString() : 'the latest'} {jobType === 'Remote' ? 'remote and work from home' : jobType.toLowerCase()} job opportunities across Kenya
            </p>
          </div>
        </div>
      </section>
...
export default function TypeJobsClient({ jobType, typeSlug }: TypeJobsClientProps) {
  return (
    <div className="min-h-screen flex flex-col">
      <Header />
      <Suspense fallback={<TypeJobsSkeleton />}>
        <TypeJobsInner jobType={jobType} typeSlug={typeSlug} />
      </Suspense>
      <Footer />
      <WhatsAppFloat />
    </div>
