...
import { MapPin, Briefcase, Loader2, AlertCircle, ChevronRight } from 'lucide-react';
...
interface CountyJobsClientProps {
  countyName: string;
  countySlug: string;
}

function CountyJobsSkeleton() {
  return (
...
function CountyJobsInner({ countyName, countySlug }: CountyJobsClientProps) {
  ...
  // Fetch jobs pre-filtered by county
      ...
      <section className="bg-gradient-to-br from-white to-gray-50 py-10 lg:py-14 border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <nav className="flex items-center gap-1.5 text-sm text-gray-400 mb-6">
            ...
            <ChevronRight className="w-3.5 h-3.5" />
            <span className="text-gray-700 font-medium">{countyName} County</span>
          </nav>

          <div className="text-center mb-6">
            <h1 className="text-3xl lg:text-4xl font-bold text-slate-800 mb-3 flex items-center justify-center gap-3">
              <MapPin className="w-8 h-8 text-teal-600" />
              Jobs in {countyName} County
            </h1>
            <p className="text-gray-500 max-w-xl mx-auto">
              Browse {total > 0 ? total.toLocaleString() : 'the latest'} job vacancies in {countyName} County, Kenya
            </p>
          </div>
        </div>
      </section>
              ...
              {!loading && !error && jobs.length === 0 && (
                <div className="text-center py-16">
                  ...
                  <h3 className="text-lg font-semibold text-slate-800 mb-2">
                    No jobs currently available in {countyName}
                  </h3>
...
export default function CountyJobsClient({ countyName, countySlug }: CountyJobsClientProps) {
  return (
    <div className="min-h-screen flex flex-col">
      <Header />
      <Suspense fallback={<CountyJobsSkeleton />}>
        <CountyJobsInner countyName={countyName} countySlug={countySlug} />
      </Suspense>
      <Footer />
      <WhatsAppFloat />
    </div>
